...
public class SynheartWear {
    ...
    private var fitbitProvider: FitbitProvider?
    private var ouraProvider: OuraProvider?
    ...
    /// Fitbit cloud provider (OAuth + data fetch via Synheart Wear API)
    public var fitbit: FitbitProvider? { return fitbitProvider }

    /// Oura ring cloud provider (OAuth + data fetch via Synheart Wear API)
    public var oura: OuraProvider? { return ouraProvider }
    ...
    public init(config: SynheartWearConfig = SynheartWearConfig(), garminHealth: GarminHealth? = nil) {
        ...
        if let appId = config.appId {
            ...
            if config.enabledAdapters.contains(.fitbit) {
                self.fitbitProvider = FitbitProvider(
                    appId: appId,
                    baseUrl: baseUrl,
                    redirectUri: redirectUri
                )
            ...
            if config.enabledAdapters.contains(.oura) {
                self.ouraProvider = OuraProvider(
                    appId: appId,
                    baseUrl: baseUrl,
                    redirectUri: redirectUri
                )
    ...
    public func getProvider(_ adapter: DeviceAdapter) throws -> WearableProvider {
        ...
        case .garmin:
            ...
            return provider
        case .fitbit:
            guard let provider = fitbitProvider else {
                throw SynheartWearError.apiError("Fitbit provider not configured. Add .fitbit to SynheartWearConfig.enabledAdapters and provide appId.")
            }
            return provider
        case .oura:
            guard let provider = ouraProvider else {
                throw SynheartWearError.apiError("Oura provider not configured. Add .oura to SynheartWearConfig.enabledAdapters and provide appId.")
            }
            return provider
        case .platformHealth, .bleHrm:
    ...
    public func readMetricsFromProvider(
        ...
        case .fitbit:
            guard let provider = fitbitProvider else {
                throw SynheartWearError.apiError("Fitbit provider not configured.")
            }
            guard provider.isConnected() else { throw SynheartWearError.notConnected }
            return try await provider.fetchActivity(start: start, end: end, limit: limit)
        case .oura:
            guard let provider = ouraProvider else {
                throw SynheartWearError.apiError("Oura provider not configured.")
            }
            guard provider.isConnected() else { throw SynheartWearError.notConnected }
            return try await provider.fetchReadiness(start: start, end: end, limit: limit)
        }
    }
