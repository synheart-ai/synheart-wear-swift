...
public enum DeviceAdapter {
    /// Platform-native health store: Apple HealthKit on iOS,
    /// Health Connect on Android (via the Flutter binding).
    case platformHealth
    case fitbit
    case garmin
    case oura
...
/// Unified biometric data structure emitted by every wear adapter.
...
public struct SynheartWearConfig {
    ...
    public init(
        enabledAdapters: Set<DeviceAdapter> = [.platformHealth],
