...
internal struct WearServiceAPI {
    ...
    // MARK: - Generic Vendor (Fitbit / Oura) OAuth + Data
    //
    // Fitbit and Oura both follow Flutter's URL shape:
    //   POST /v1/oauth/{vendor}/initiate
    //   GET  /v1/{vendor}/data?user_id=...&data_type=...
    //   POST /v1/{vendor}/disconnect

    func initiateVendorOAuth(
        vendor: String,
        userId: String,
        appId: String,
        redirectUri: String,
        projectId: String? = nil
    ) async throws -> VendorOAuthInitiateResponse {
        let body = VendorOAuthInitiateRequest(
            userId: userId,
            redirectUri: redirectUri,
            appId: appId,
            projectId: projectId
        ...
        return try await networkClient.post(
            path: "/v1/oauth/\(vendor)/initiate",
            body: body
        )
    }

    func fetchVendorData(
        vendor: String,
        userId: String,
        dataType: String,
        appId: String,
        ...
        limit: Int? = nil,
        projectId: String? = nil
    ) async throws -> VendorDataResponse {
        var queryParams: [String: String] = [
            "user_id": userId,
            "data_type": dataType,
            "app_id": appId
        ]
        if let limit = limit { queryParams["limit"] = String(limit) }
        if let start = start {
            queryParams["start"] = ISO8601DateFormatter().string(from: start)
        }
        if let end = end {
            queryParams["end"] = ISO8601DateFormatter().string(from: end)
        }
    ...
    func disconnectVendor(
        vendor: String,
        userId: String,
        appId: String
    ) async throws -> DisconnectResponse {
        let body = VendorDisconnectRequest(userId: userId, appId: appId)
        return try await networkClient.post(
            path: "/v1/\(vendor)/disconnect",
            body: body
        )
    }
...
// MARK: - Vendor (Fitbit / Oura) Models

internal struct VendorOAuthInitiateRequest: Codable {
    let userId: String
    let redirectUri: String
    let appId: String
...
internal struct VendorOAuthInitiateResponse: Codable {
    let authUrl: String
    let state: String
...
internal struct VendorDisconnectRequest: Codable {
    let userId: String
    let appId: String
...
/// Generic vendor (Fitbit / Oura) data response. Records are flat envelopes.
internal struct VendorDataResponse: Codable {
    let records: [DataRecord]
    let cursor: String?
