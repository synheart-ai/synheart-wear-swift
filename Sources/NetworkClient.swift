        ...
        do {
            ...
            case 200...299:
                ...
                do {
                    ...
                    // Surface a structured error without dumping the response
                    // body — the body may include biometric values, vendor
                    // user IDs, or OAuth state.
                    var errorContext = "Decoding error: \(decodingError.localizedDescription)"
                    switch decodingError {
                    case .keyNotFound(let key, let context):
                        errorContext += " — missing key: \(key.stringValue) at path: \(context.codingPath.map { $0.stringValue }.joined(separator: "."))"
                    case .typeMismatch(let type, let context):
                        errorContext += " — type mismatch: expected \(type) at path: \(context.codingPath.map { $0.stringValue }.joined(separator: "."))"
                    case .valueNotFound(let type, let context):
                        errorContext += " — value not found: expected \(type) at path: \(context.codingPath.map { $0.stringValue }.joined(separator: "."))"
                    case .dataCorrupted(let context):
                        errorContext += " — data corrupted at path: \(context.codingPath.map { $0.stringValue }.joined(separator: ".")) - \(context.debugDescription)"
                    @unknown default:
                        break
                    }
