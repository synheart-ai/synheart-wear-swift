...
2. Enter the Swift Package URL provided by Garmin Health Support with your SDK access.
3. Authenticate with the credentials Garmin issued.
...
1. **Download the SDK**
   ...
   Download the `Companion.xcframework-X.X.X.zip` from your Garmin
...
let synheart = SynheartWear(
    config: SynheartWearConfig(enabledAdapters: [.garmin]),
    garminHealth: garmin
)
