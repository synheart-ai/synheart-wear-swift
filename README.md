...
[![watchOS 8.0+](https://img.shields.io/badge/watchOS-8.0%2B-blue.svg)](https://developer.apple.com/watchos/)
...
let config = SynheartWearConfig(
    enabledAdapters: [.platformHealth, .whoop],
    enableLocalCaching: true,
    enableEncryption: true,
    streamInterval: 3.0,
...
Every adapter emits a unified `WearMetrics` value:
...
- **HealthKit-aligned**: Follows Apple's HealthKit guidelines for biometric data handling
...
This repository is **source-available**. Pull requests are not accepted — see
[CONTRIBUTING.md](CONTRIBUTING.md) for the rationale and how to contribute via
issues. Security reports go through [SECURITY.md](SECURITY.md).
...
Full reference docs live at **[docs.synheart.ai/synheart-wear/swift](https://docs.synheart.ai/synheart-wear/swift)** — provider catalog, streaming semantics, architecture, BLE HRM setup, Apple Health XML import, error reference, and the cross-platform data schema.
